...
name = [r'Fls30/fl min$\mathregular{^{-1}}$' + r'100km$\mathregular{^{2}}$',
        'Maxht40/km', r'Vpx40/km$\mathregular{^{3}}$']
...
for i in all_list[1:]:
    ...
    zuizhongy.reverse()
    # 创建一个DataFrame对象开始绘图
    df = pd.DataFrame({name[k]: zuizhongy, 'r': rx})
...
# 利用seaborn库绘制箱型图，注意这里我们一定要把zuizhongy和rx求出来就是因为boxplot只能对一维海量数据进行箱型图绘制，注意的是这里的x轴的值可以是字符串
    ...
    g = sns.boxplot(x="r", y=name[k], whis=0.6, data=df, showfliers=False, color="goldenrod", width=0.3)
    ...
    # 这个可以设置y轴的刻度距离是多少
    if k == 2:
        g.set_xlabel("r", font={"family": "Times New Roman", "size": 80})
    else:
    ...
    all = df.groupby('r', as_index=False).mean()
    y = all[name[k]]
    x = all["r"]
    # 验证y中是空值的位置
    w1 = np.isfinite(y)
    # 连接空值左右两边的点画折线图
    g.plot(x[w1], y[w1], linestyle='-', linewidth=0.8, color="black", label="average_line", marker=".",
           markersize="10")
    g.set_xlim(-1, 33.5)
    # 隐藏y轴的坐标轴标题
    g.set(ylabel=None)
    ...
    g.text(0.92, 0.86, legend[k], transform=g.transAxes, fontsize=100)
    k += 1
