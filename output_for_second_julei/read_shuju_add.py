...
def read_shuju_ADD():
    ...
    npixels_40 = np.multiply(npixels_40, 4)
    npixels_40 = np.divide(npixels_40, math.pi)
    npixels_40_R = np.sqrt(npixels_40)
    npixels_30 = data.select("npixels_30")[:]
    npixels_30 = npixels_30[index]
    npixels_30 = npixels_size(npixels_30, boost)
    npixels_30 = np.multiply(npixels_30, 4)
    npixels_30 = np.divide(npixels_30, math.pi)
    npixels_30_R = np.sqrt(npixels_30)
    npixels_20 = data.select("npixels_20")[:]
    npixels_20 = npixels_20[index]
    npixels_20 = npixels_size(npixels_20, boost)
    npixels_20 = np.multiply(npixels_20, 4)
    npixels_20 = np.divide(npixels_20, math.pi)
    npixels_20_R = np.sqrt(npixels_20)
    # 闪电频数=闪电数/viewtime*60，我们使用每分钟的闪电频数
    flashfrequence = np.divide(flashcount * 60, viewtime)
    flash_20 = np.divide(npixels_20, flashfrequence * 100)
    flash_30 = np.divide(npixels_30, flashfrequence * 100)
    flash_40 = np.divide(npixels_40, flashfrequence * 100)
    maxht20_minux_maxht40 = maxht20 - maxht40
    ellip_20 = np.divide(npixels_20_R * 2, maxht20_minux_maxht40)
    ellip_30 = np.divide(npixels_30_R * 2, maxht20_minux_maxht40)
    ellip_40 = np.divide(npixels_40_R * 2, maxht20_minux_maxht40)
    ...
    r = np.divide(rainconv, volrain)
    index_add1 = list(np.where(r > 0.64))
    ...
    npx40_divide_npx20 = np.divide(npixels_40, npixels_20)
    return r, maxht20, maxht30, maxht40, npixels_20, npixels_30, npixels_40, flash_20, flash_30, flash_40, minir,\
        flashrate, maxht20_minux_maxht40, ellip_20, ellip_30, ellip_40, maxdbz, maxht, npx40_divide_npx20
