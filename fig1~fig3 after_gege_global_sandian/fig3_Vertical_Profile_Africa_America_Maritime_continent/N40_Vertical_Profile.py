...
# overall average
mdbz_africa = np.mean(chuli_mianji(data1[1], data1[4]), axis=0)
mdbz_americas = np.mean(chuli_mianji(data2[1], data2[4]), axis=0)
mdbz_Maritime_continent = np.mean(chuli_mianji(data3[1], data3[4]), axis=0)
# top10% average
# mdbz_africa_average = top10(chuli_mianji(data1[1], data1[4]))
# mdbz_americas_average = top10(chuli_mianji(data2[1], data2[4]))
# mdbz_Maritime_continent_average = top10(chuli_mianji(data3[1], data3[4]))
...
# 加了top10%的
# ax.set_xticks(np.arange(0, 3650, 600))
# 这是没有加top10%的
ax.set_xlim(0, 555)
# 这是加了top10%的
# ax.set_xlim(0, 3650)
ax.set_ylim(0, 21)
ax.text(0.05, 0.9, "(e)", transform=ax.transAxes, fontsize=50)
...
plt.savefig(f"C:\\Users\\lvyih\\Desktop\\Africa_Americas_Maritime_continent_S40.jpeg", bbox_inches="tight", dpi=400)
"""ax.text(
        0.2, 0.1, 'some text',
