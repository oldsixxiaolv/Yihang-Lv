...
def read_shuju():
    ...
    for qqq in [index_Africa, index_Nanmei, index_Yindunixia]:
        ...
        mdbz = mdbz[inded]
        r_mid = r_mid[inded]
        boost_mid = boost_mid[inded]
        mid_list = []
        mid_list.append(n20)
        mid_list.append(n40)
        mid_list.append(mdbz)
        mid_list.append(r_mid)
        mid_list.append(boost_mid)
...
# overall average
mdbz_africa = np.mean(chuli_mianji(data1[0], data1[4]), axis=0)
mdbz_americas = np.mean(chuli_mianji(data2[0], data2[4]), axis=0)
mdbz_Maritime_continent = np.mean(chuli_mianji(data3[0], data3[4]), axis=0)
# top10% average
# mdbz_africa_top10 = top10(chuli_mianji(data1[0], data1[4]))
# mdbz_americas_top10 = top10(chuli_mianji(data2[0], data2[4]))
# mdbz_Maritime_continent_top10 = top10(chuli_mianji(data3[0], data3[4]))
...
# top10% average
# ax.plot(mdbz_africa_top10, Height, "r", marker="^",
#         linewidth=4.33, markersize=12)
# ax.plot(mdbz_americas_top10, Height, "b", marker="s",
#         linewidth=4.33, markersize=12)
# ax.plot(mdbz_Maritime_continent_top10, Height, "g", marker="o",
#         linewidth=4.33, markersize=12)
# 尽管我们更改了整个图形的尺寸，但是可以通过以下set_xticks进行x轴标记的手动设置。
# 以下是没有加top10%的
ax.set_xticks(np.arange(0, 6000, 1000))
# 这是加了top10%的
# ax.set_xticks(np.arange(0, 34000, 6400))
# 这是没有加top10%的x轴的范围
ax.set_xlim(0, 5425)
# 这是加了top10%的x轴的范围
# ax.set_xlim(0, 34000)
ax.set_ylim(0, 21)
ax.text(0.05, 0.9, "(f)", transform=ax.transAxes, fontsize=50)
