...
def read_shuju():
    ...
    # 两个数组取交集
    index = list(set(index11[0]) & set(index12[0]) & set(index13[0]) & set(index2[0]) & set(index5[0])
                 & set(index6[0]) & set(index7[0]))
    index = sorted(index)
    # 把hdf文件中的闪电频数数据提取出来
    flashcount = data.select('flashcount')[:]
    ...
    return latitude, longitude, maxht20, minir, maxht40, flashrate
