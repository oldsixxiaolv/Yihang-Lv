...
def read_shuju():
    ...
    # 筛选数据
    index1 = np.where(longitude > -110)
    index2 = list(np.where(latitude > -30))
    index3 = list(np.where(landocean == 1))
    index4 = list(np.where(longitude < -40))
    index5 = list(np.where(latitude < 30))
    ...
    # 两个数组取交集
    index = list(set(index1[0]) & set(index11[0]) & set(index12[0]) & set(index13[0]) & set(index2[0]) & set(index5[0])
                 & set(index3[0]) & set(index4[0]) & set(index6[0]) & set(index7[0]))
    index = sorted(index)
    # 把hdf文件中的闪电频数数据提取出来
    flashcount = data.select('flashcount')[:]
