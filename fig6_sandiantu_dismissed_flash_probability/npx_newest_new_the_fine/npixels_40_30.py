...
from color_characterized_3040 import color_characterized
...
w = 0
hh = 0
npixels40, npixels30, _color_, allshuju = color_characterized(flashcount, npixels_40, npixels_30, w, hh)
ax.scatter(npixels40, npixels30, s=2, color=_color_, marker=".")
ax.vlines(npixels40_50, 0, 1500, linestyles="dashed", color="black", label="50%")
ax.vlines(npixels40_80, 0, 1500, linestyles="dashed", color="red", label="80%")
# 创建绘制等值线图
x = np.arange(0, 600, 25)
y = np.arange(0, 1500, 15)
all_scale = []
print("third")
for i in y:
    two_scale = []
    for j in x:
        ...
        for (u, v) in zip(npixels_40, npixels_30):
            if abs(i - v) <= 7.5 and abs(j - u) <= 12.5:
                mid += 1
...
ax.set_xlabel("npx40/km$^{2}$", fontsize=14)
ax.set_ylabel("npx30/km$^{2}$", fontsize=14)
...
ax.minorticks_on()
# 这是打开次刻度的代码
plt.savefig(r"C:\\Users\\lvyih\\Desktop\\all_stage_npx4030.jpeg", dpi=400)
