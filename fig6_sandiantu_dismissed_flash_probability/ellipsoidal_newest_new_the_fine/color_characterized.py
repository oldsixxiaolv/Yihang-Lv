...
def color_characterized(flash, ht30, ht20, w, hh):
    ...
    for i in np.arange(0, 1, 0.05):
        for j in np.arange(0, 1, 0.05):
            index1 = list(np.where(np.array(htt30) - i < 0.025))
            index2 = list(np.where(np.array(htt20) - j < 0.025))
            index3 = list(np.where(np.array(htt30) - i > -0.025))
            index4 = list(np.where(np.array(htt20) - j > -0.025))
